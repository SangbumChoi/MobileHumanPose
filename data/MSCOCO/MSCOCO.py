...
class MSCOCO:
    def __init__(self, data_split):
        ...
        _abs = osp.join('/', 'home', 'centos', 'datasets', 'coco')
        _root = _abs if osp.isdir(_abs) else osp.dirname(osp.abspath(__file__))
        self.img_dir = osp.join(_root, 'images')
        self.train_annot_path = osp.join(_root, 'annotations', 'person_keypoints_train2017.json')
        self.test_annot_path = osp.join(_root, 'annotations', 'person_keypoints_val2017.json')
        self.human_3d_bbox_root_dir = osp.join(_root, 'bbox_root', 'bbox_root_coco_output.json')
