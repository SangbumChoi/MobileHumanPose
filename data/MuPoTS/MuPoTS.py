...
class MuPoTS:
    def __init__(self, data_split):
        ...
        _abs = osp.join('/', 'data', 'MuPoTS')
        _root = _abs if osp.isdir(_abs) else osp.dirname(osp.abspath(__file__))
        self.img_dir = osp.join(_root, 'data', 'MultiPersonTestSet')
        self.test_annot_path = osp.join(_root, 'data', 'MuPoTS-3D.json')
        self.human_bbox_root_dir = osp.join(_root, 'bbox_root', 'bbox_root_mupots_output.json')
