...
def soft_argmax(heatmaps, joint_num):
    ...
    # arange must live on the same device as the heatmaps, otherwise this
    # crashes on GPU (device mismatch) and silently mis-scales nothing on CPU.
    dev = accu_x.device
    accu_x = accu_x * torch.arange(1, cfg.output_shape[1] + 1, device=dev, dtype=accu_x.dtype)
    accu_y = accu_y * torch.arange(1, cfg.output_shape[0] + 1, device=dev, dtype=accu_y.dtype)
    accu_z = accu_z * torch.arange(1, cfg.depth_dim + 1, device=dev, dtype=accu_z.dtype)
